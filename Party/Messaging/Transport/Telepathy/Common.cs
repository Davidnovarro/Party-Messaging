...
namespace Telepathy
{
    public abstract class Common
    {
        ...
        //This is used on order to prevent spamming the debug console with the same message
        private const long minLogDelayMilliseconds = 1000;

        private int prevLogErrorCode;

        private long prevLogUnixMilliseconds;
        ...
        protected bool CanLog(int logCode)
        {
            if (prevLogErrorCode != logCode)
                return true;

            long unix = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
            if (unix - prevLogUnixMilliseconds > minLogDelayMilliseconds)
            {
                prevLogErrorCode = logCode;
                prevLogUnixMilliseconds = unix;
                return true;
            }
            else
            {
